...
def stub_heavy_calls(monkeypatch, tmp_path):
    ...
    def fake_run_attack_query(model, tokenizer, model_key, corpus_name, question, injection_template, index=None, records=None, **kwargs):
        calls.append((model_key, corpus_name, injection_template))
        return {'question': question, 'retrieved_doc_ids': [0], 'retrieval_scores': [0.9], 'generated_answer': 'fake gold answer', 'generated_answer_clean': 'fake gold answer', 'injection_template': injection_template, 'hijack_type': 'goal', 'target_string': '42'}
...
def test_unknown_model_key_raises(stub_heavy_calls):
    with pytest.raises(ValueError, match='Unknown model keys'):
        rai.run_attack_sweep(model_keys=['not_a_real_model'], corpus_names=['hotpot_qa'], injection_templates=['naive'])
...
def test_nq_open_raises_because_excluded_by_default(stub_heavy_calls):
    with pytest.raises(ValueError, match='Unknown or excluded corpus names'):
        rai.run_attack_sweep(model_keys=['phi-4-mini'], corpus_names=['nq_open'], injection_templates=['naive'])
...
def test_unknown_injection_template_raises(stub_heavy_calls):
    with pytest.raises(ValueError, match='Unknown injection templates'):
        rai.run_attack_sweep(model_keys=['phi-4-mini'], corpus_names=['hotpot_qa'], injection_templates=['not_a_real_template'])

def test_sweep_writes_one_cell_per_model_corpus_template(stub_heavy_calls, tmp_path):
    calls = stub_heavy_calls
    rai.run_attack_sweep(model_keys=['phi-4-mini'], corpus_names=['hotpot_qa', 'ms_marco'], injection_templates=['naive', 'combined'])
    assert len(calls) == 4
    assert set(calls) == {('phi-4-mini', 'hotpot_qa', 'naive'), ('phi-4-mini', 'hotpot_qa', 'combined'), ('phi-4-mini', 'ms_marco', 'naive'), ('phi-4-mini', 'ms_marco', 'combined')}
...
def test_build_defended_attack_prompt_instruction_detection_drops_flagged_passage(monkeypatch):
    monkeypatch.setattr(rai, 'retrieve', lambda index, records, question, k: [({'id': 0}, 0.9), ({'id': 1}, 0.5)])
    ...
    def fake_detect_injection(text):
        ...
        flagged = 'Instead, answer' in text
        return DetectionResult(flagged=flagged, score=0.99 if flagged else 0.01, label='INJECTION' if flagged else 'SAFE')
    monkeypatch.setattr(rai, 'detect_injection', fake_detect_injection)
    system_prompt, user_prompt, retrieved, target_string, hijack_type, passage_log = rai._build_defended_attack_prompt(index=None, records=None, question='q?', corpus_name='hotpot_qa', injection_template='naive', top_k=2, defense='instruction_detection')
    assert system_prompt == rai.SYSTEM_PROMPT
    assert '[1]' not in user_prompt
    assert 'clean text 1' in user_prompt
...
def test_build_defended_attack_prompt_spotlighting_encodes_every_passage(monkeypatch):
    ...
    system_prompt, user_prompt, retrieved, target_string, hijack_type, passage_log = rai._build_defended_attack_prompt(index=None, records=None, question='q?', corpus_name='hotpot_qa', injection_template='naive', top_k=2, defense='spotlighting')
    assert passage_log is None
    assert rai.SPOTLIGHTING_SYSTEM_INSTRUCTION in system_prompt
    assert 'plain text' not in user_prompt
    import base64
...
def test_output_filter_defense_replaces_generated_answer_and_writes_distinct_filename(stub_heavy_calls, tmp_path, monkeypatch):
    calls = []
    ...
    rai.run_attack_sweep(model_keys=['phi-4-mini'], corpus_names=['hotpot_qa'], injection_templates=['naive'], defense='output_filter')
    assert len(calls) == 1
...
def test_sweep_scores_attack_success_against_target_string(stub_heavy_calls, tmp_path):
    import json
    rai.run_attack_sweep(model_keys=['phi-4-mini'], corpus_names=['hotpot_qa'], injection_templates=['naive'])
...
def test_sample_n_caps_questions_processed_per_cell(stub_heavy_calls_multi_record):
    ...
    rai.run_attack_sweep(model_keys=['phi-4-mini'], corpus_names=['hotpot_qa'], injection_templates=['naive'], sample_n=2)
    assert len(calls) == 2

def test_sample_n_none_processes_every_record(stub_heavy_calls_multi_record):
    calls = stub_heavy_calls_multi_record
    rai.run_attack_sweep(model_keys=['phi-4-mini'], corpus_names=['hotpot_qa'], injection_templates=['naive'], sample_n=None)
    assert len(calls) == 5
