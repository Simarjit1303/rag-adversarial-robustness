...
def stub_phase_a(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(rp, 'build_index', lambda corpus_name, split='dev': (mock.Mock(), [{'q': f'question {i}', 'a': [f'answer {i}']} for i in range(3)]))
    ...
    monkeypatch.setattr(rp, 'sample_target_questions', lambda records, corpus_name, sample_size, seed: records)
    monkeypatch.setattr(rp, 'generate_poison_texts', lambda q, a, api_token: ('wrong answer', ['c1', 'c2', 'c3', 'c4', 'c5']))
    ...
    monkeypatch.setattr(rp, 'retrieve_with_poison', lambda index, records, poison_texts, poison_embeddings, query, k: ([('real doc', 0.5, 0), (poison_texts[0], 0.9, 'poison::0')], {'poison::0', 'poison::1'}))
...
def test_build_poisoned_contexts_produces_one_entry_per_target_question(stub_phase_a):
    ...
    assert contexts[0]['retrieval_f1'] == 0.5

def test_build_poisoned_contexts_skips_a_question_whose_generation_exhausts_retries(stub_phase_a, monkeypatch):
    monkeypatch.setattr(rp.time, 'sleep', lambda s: None)
    calls = {'n': 0}
...
def test_build_poisoned_contexts_caches_to_disk_and_reuses_on_next_call(stub_phase_a, tmp_path):
    ...
    assert second == first
    assert calls_after_cache == []

def test_build_poisoned_contexts_regenerates_when_cache_is_smaller_than_requested(stub_phase_a, monkeypatch, tmp_path):
    monkeypatch.setattr(rp, 'build_index', lambda corpus_name, split='dev': (mock.Mock(), [{'q': f'question {i}', 'a': [f'answer {i}']} for i in range(5)]))
    monkeypatch.setattr(rp, 'sample_target_questions', lambda records, corpus_name, sample_size, seed: records[:sample_size])
...
_CTX = {'question': 'q0', 'gold_answers': ['gold'], 'target_answer': 'wrong', 'retrieved_doc_ids': [0], 'retrieval_precision': 1.0, 'retrieval_recall': 1.0, 'retrieval_f1': 1.0}

def test_build_row_flags_poison_echoed_not_adopted_only_when_diagnostic_hit_but_em_missed():
    scores = {'attack_success': 0, 'f1_target': 0.57, 'contains_target_diagnostic': 1, 'em_gold': 0, 'f1_gold': 0.1}
...
def test_build_row_does_not_flag_a_true_attack_success():
    scores = {'attack_success': 1, 'f1_target': 1.0, 'contains_target_diagnostic': 1, 'em_gold': 0, 'f1_gold': 0.1}
    row = rp._build_row('m', 'hotpot_qa', 'adv5', _CTX, 'gen', 'gen_clean', scores)
    assert row['poison_echoed_not_adopted'] == 0

def test_build_row_does_not_flag_no_poison_influence_at_all():
    scores = {'attack_success': 0, 'f1_target': 0.0, 'contains_target_diagnostic': 0, 'em_gold': 1, 'f1_gold': 1.0}
    row = rp._build_row('m', 'hotpot_qa', 'adv5', _CTX, 'gen', 'gen_clean', scores)
    assert row['poison_echoed_not_adopted'] == 0

def test_summarize_reports_poison_echoed_not_adopted_rate_alongside_attack_success_rate():
    rows = [{'attack_success': 1, 'f1_target': 1.0, 'contains_target_diagnostic': 1, 'poison_echoed_not_adopted': 0, 'exact_match': 0, 'f1_clean': 0.1, 'retrieval_f1': 1.0}, {'attack_success': 0, 'f1_target': 0.5, 'contains_target_diagnostic': 1, 'poison_echoed_not_adopted': 1, 'exact_match': 0, 'f1_clean': 0.2, 'retrieval_f1': 1.0}, {'attack_success': 0, 'f1_target': 0.0, 'contains_target_diagnostic': 0, 'poison_echoed_not_adopted': 0, 'exact_match': 1, 'f1_clean': 1.0, 'retrieval_f1': 1.0}]
...
def stub_full_sweep(monkeypatch, tmp_path):
    monkeypatch.setattr(rp, 'RESULTS_DIR', tmp_path)
    monkeypatch.setattr(rp, 'build_poisoned_contexts', lambda corpus_name, split='dev', sample_n=None, poison_config='adv5', **kw: [{'question': 'q0', 'gold_answers': ['gold'], 'target_answer': 'wrong', 'context': '[1] ctx', 'retrieved_doc_ids': [0], 'retrieval_precision': 1.0, 'retrieval_recall': 1.0, 'retrieval_f1': 1.0}])
    calls = []
...
def test_sweep_loads_each_model_once_and_writes_a_summary_per_cell(stub_full_sweep, tmp_path):
    ...
    summary_rows = rp.run_poisonedrag_sweep(model_keys=['phi-4-mini', 'qwen3-8b'], corpus_names=['hotpot_qa', 'ms_marco'], poison_configs=['adv5'])
...
def test_render_defended_poisoned_context_instruction_detection_drops_flagged_poison_entry(monkeypatch):
    ...
    def fake_detect_injection(text):
        ...
        return DetectionResult(flagged=flagged, score=0.9 if flagged else 0.1, label='INJECTION' if flagged else 'SAFE')
...
def test_output_filter_defense_replaces_generated_answer_and_writes_distinct_filename(stub_full_sweep, tmp_path, monkeypatch):
    calls = []
    ...
    rp.run_poisonedrag_sweep(model_keys=['phi-4-mini'], corpus_names=['hotpot_qa'], poison_configs=['adv5'], defense='output_filter')
    assert len(calls) == 1
