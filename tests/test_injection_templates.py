"""
Pins the five injection templates' exact construction (Liu et al.'s formal
notation, adapted -- see attacks/injection_templates.py's module docstring)
and this project's goal/process hijack-type mapping. These are the pinned
template set: a test failure here after an edit means the template text
changed under the same key, which silently invalidates any cached
attack_raw_*.jsonl results keyed by that name.
"""

from attacks.injection_templates import (
    GOAL_HIJACK_TARGET,
    PROCESS_HIJACK_TARGET,
    TEMPLATES,
)
...
def test_naive_has_no_separator():
    ...
    assert "\n" not in rendered
    assert GOAL_HIJACK_TARGET not in DOC_TEXT  # sanity: canary doesn't collide with fixture text
...
def test_every_template_preserves_the_organic_document_text():
    # the attack must never DROP the original organic content -- only
    # append to it, per "organic placement" (this attack doesn't touch
    # what gets retrieved, only appends to a retrieved doc's text)
