"""
Self-check for scripts/clean_scratch_volume.py's filtering logic -- the part
that decides what's a deletion candidate. Uses a hand-rolled fake S3 client
(duck-typing get_paginator().paginate()) so this never needs real boto3
credentials or network access; only main()'s CLI wiring touches real boto3.
"""
...
def test_real_result_files_are_never_candidates_regardless_of_age():
    s3 = _FakeS3([
        _obj("results/baseline_raw_phi-4-mini_hotpot_qa_vllm.jsonl", age_hours=999),
        _obj("results/attack_summary_ministral-3-8b_ms_marco_ignore_vllm.csv", age_hours=999),
        _obj("phase1_results_complete/baseline_raw_llama-3.1-8b_ms_marco_vllm.jsonl", age_hours=999),
    ])
...
def test_never_delete_safety_net_catches_a_tmp_prefixed_protected_name():
    # Adversarial edge case: starts with .tmp_ (passes the inclusion filter)
    # but its remainder matches a NEVER_DELETE pattern (*.md) -- the
    # belt-and-suspenders exclusion check must still veto it.
