...
%     into vague hedges:
%     - nq_open exclusion: CONFIRMED AND APPROVED by supervisor -- state
%       as a resolved, closed scope decision here, not a pending item.
%       Still list it under Limitations (a real limitation of the
%       corpus coverage, even though resolved procedurally) with the
%       full technical reasoning from the Evaluation chapter's nq_open
%       section -- don't just write "excluded, approved by supervisor"
%       and move on; the corpus-construction fact is what an examiner
%       needs to see argued, not asserted
