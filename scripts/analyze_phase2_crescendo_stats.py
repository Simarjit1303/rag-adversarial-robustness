"""
Phase 2 Attack 3 (Crescendo) statistical analysis.

Reads phase2_crescendo_real_sweep/ directly by path -- same pattern as
scripts/analyze_phase2_injection_stats.py and
scripts/analyze_phase2_poisonedrag_stats.py. Unlike those two attacks,
Crescendo has no RAG retrieval step and no per-corpus dimension: one
target-model x one attack (multi-turn escalation) x one behavior pool
(jbb_behaviors + harmbench, sampled to n=100, seed=SEED -- see
evaluation/run_crescendo.py's run_crescendo_sweep) x 5 turns, 4 cells total
(one per model).

All 4 models are run against the SAME sampled 100 behaviors (identical
pool, identical sample_behaviors(..., seed=SEED) call for every model), so
model-vs-model attack_success is a genuinely PAIRED comparison keyed by
behavior text -- McNemar's exact test, one family of C(4,2)=6 pairs,
Holm-Bonferroni corrected. judge_failed rows (API/parse failures, not a
target-model outcome -- same "skip, don't count as failure" precedent as
the other two attacks) are excluded from the ASR denominator and from any
pairwise comparison touching that behavior for that model.

Run: python -m scripts.analyze_phase2_crescendo_stats
"""
...
from evaluation.stats import holm_bonferroni, mcnemar_exact, paired_bootstrap_ci  # noqa: E402
...
def main():
    ...
    for m in model_keys:
        ...
        print(f"  {m:16s} n={n} n_scored={n_scored} asr={asr:.4f} "
              f"backtrack_rate={backtrack_rate:.4f} mean_backtrack={mean_backtrack:.3f} "
              f"mean_refusal={mean_refusal:.3f}")
    ...
    for a, b in itertools.combinations(model_keys, 2):
        shared = [
            beh for beh in data[a]
            if beh in data[b] and not data[a][beh]["judge_failed"] and not data[b][beh]["judge_failed"]
        ]
    ...
    for (a, b, n_paired, result), label, holm in zip(tables, labels, corrected):
        print(f"  {label:34s} n_paired={n_paired:3d} b={result['b']:2d} c={result['c']:2d} "
              f"p_raw={result['p_value']:.4f} rank={holm['rank']} "
              f"significant_holm={holm['significant_holm']}")
