...
def main():
    ...
    for corpus in corpora:
        for model in model_keys:
            ...
            if len(common_qs) != len(attack):
                print(f"WARNING: {model}/{corpus}: {len(attack) - len(common_qs)} attack questions had no baseline match -- excluded from this cell's stats")
            ...
            cell_stats[model, corpus] = {'n': len(common_qs), 'asr': sum(asr) / len(asr) if asr else float('nan'), 'echoed_not_adopted_rate': sum(echoed) / len(echoed) if echoed else float('nan'), 'contains_target_rate': sum(contains_target) / len(contains_target) if contains_target else float('nan'), 'f1_baseline': sum(baseline_f1) / len(baseline_f1), 'f1_attack': sum(attack_f1) / len(attack_f1), 'utility_drop': boot['observed_drop'], 'utility_drop_ci_low': boot['ci_low'], 'utility_drop_ci_high': boot['ci_high']}
    ...
    header = f"{'model':<16}{'corpus':<10}{'n':>5}{'ASR':>8}{'echoed':>8}{'contains':>9}{'F1_base':>9}{'F1_atk':>8}{'drop':>8}{'ci_lo':>8}{'ci_hi':>8}"
    print(header)
    for corpus in corpora:
        for model in model_keys:
            s = cell_stats[model, corpus]
            print(f"{model:<16}{corpus:<10}{s['n']:>5}{s['asr']:>8.4f}{s['echoed_not_adopted_rate']:>8.4f}{s['contains_target_rate']:>9.4f}{s['f1_baseline']:>9.4f}{s['f1_attack']:>8.4f}{s['utility_drop']:>8.4f}{s['utility_drop_ci_low']:>8.4f}{s['utility_drop_ci_high']:>8.4f}")
