...
BASELINE_SUMMARY_FIELDS = ['model', 'corpus', 'n', 'f1_clean', 'exact_match', 'f1_raw', 'contains_answer_diagnostic']
ATTACK_SUMMARY_FIELDS = ['model', 'corpus', 'injection_template', 'n', 'f1_clean', 'exact_match', 'f1_raw', 'contains_answer_diagnostic', 'attack_success_rate']
...
def recompute_file(raw_path: str, summary_path: str, extra_keys: list[str], summary_fields: list[str], dry_run: bool) -> tuple[int, int]:
    with open(raw_path, encoding='utf-8') as f:
        rows = [json.loads(line) for line in f]
    changed = 0
    for row in rows:
        before = (row['exact_match'], row['f1_clean'], row['f1_raw'], row['contains_answer_diagnostic'])
        recompute_row(row)
        after = (row['exact_match'], row['f1_clean'], row['f1_raw'], row['contains_answer_diagnostic'])
        if before != after:
            changed += 1
    if not dry_run:
...
def main():
    ...
    for raw_path in sorted(glob.glob(os.path.join(BASELINE_DIR, 'baseline_raw_*.jsonl'))):
        ...
        n, changed = recompute_file(raw_path, summary_path, ['model', 'corpus'], BASELINE_SUMMARY_FIELDS, args.dry_run)
        total_files += 1
        total_rows += n
    ...
    for raw_path in sorted(glob.glob(os.path.join(ATTACK_DIR, 'attack_raw_*.jsonl'))):
        ...
        n, changed = recompute_file(raw_path, summary_path, ['model', 'corpus', 'injection_template'], ATTACK_SUMMARY_FIELDS, args.dry_run)
        total_files += 1
        total_rows += n
        total_changed += changed
        if changed:
            print(f'  {raw_path}: {changed}/{n} rows changed')
    print(f"\n{('[dry-run] ' if args.dry_run else '')}{total_files} files, {total_rows} rows, {total_changed} rows changed")
